...
const SOURCE_COLORS = {
  ...
  'Email':       'var(--accent-violet)',
  'Referral':    'var(--accent-amber)',
  'Social Media':'var(--accent-rose)',
  'Direct':      'var(--accent-teal)',
};
...
function OverviewTab({ data }) {
  const weeklyChart = data.weeklyVisitors.map(d => ({
    label: d.day, value: d.visitors, color: 'var(--accent-violet)',
  }));
  ...
  return (
    <>
      <section className="analytics-charts">
        <Card title="Weekly Visitors" subtitle="Last 7 days">
      ...
      <Card title="Traffic Source Detail" subtitle="Full breakdown">
        <table className="data-table">
          <thead>
            <tr><th>Source</th><th>Visitors</th><th>Share</th></tr>
          </thead>
          <tbody>
...
function SocialMediaTab({ sm }) {
  const impressionsChart = sm.monthlyImpressions.map(m => ({
    label: m.month,
    value: m.linkedin + m.facebook + m.instagram,
          ...
          <div key={p.platform} className="platform-card">
            ...
            <div className="platform-metrics">
              <div className="pm-item">
                <span className="pm-value">{p.followers.toLocaleString()}</span>
                <span className="pm-label">Followers</span>
              </div>
              <div className="pm-item">
                <span className="pm-value">{p.impressions.toLocaleString()}</span>
                <span className="pm-label">Impressions</span>
              </div>
              <div className="pm-item">
                <span className="pm-value">{p.engagementRate}</span>
                <span className="pm-label">Engagement</span>
              </div>
              <div className="pm-item">
                <span className="pm-value">{p.clicks.toLocaleString()}</span>
                <span className="pm-label">Clicks</span>
              </div>
            </div>
            <div className="platform-footer">
              <span>{p.posts} posts this month</span>
            </div>
          </div>
        ))}
      </section>
        ...
        <table className="data-table">
          <thead>
            <tr>
              <th>Month</th>
              <th>
                <span className="platform-dot" style={{ background: PLATFORM_COLORS['LinkedIn'] }} />
                LinkedIn
              </th>
              <th>
                <span className="platform-dot" style={{ background: PLATFORM_COLORS['Facebook'] }} />
                Facebook
              </th>
              <th>
                <span className="platform-dot" style={{ background: PLATFORM_COLORS['Instagram'] }} />
                Instagram
              </th>
              <th>Total</th>
            </tr>
          ...
          <tbody>
            {sm.monthlyImpressions.map((m, i) => (
              <tr key={i}>
                <td className="td-primary">{m.month}</td>
                <td className="td-number">{m.linkedin.toLocaleString()}</td>
                <td className="td-number">{m.facebook.toLocaleString()}</td>
                <td className="td-number">{m.instagram.toLocaleString()}</td>
                <td className="td-number td-positive">{(m.linkedin + m.facebook + m.instagram).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          {sm.topPosts.map((p, i) => (
            <div key={i} className="top-post-row">
              ...
              <div className="top-post-stats">
                ...
                <span className="td-positive">{p.engagementRate}</span>
...
function LeadTrackerTab({ leads }) {
  ...
  // source breakdown for funnel bars
    ...
    <>
      ...
      <div className="lead-summary-row">
        <div className="lead-summary-card">
          <span className="ls-value">{leads.length}</span>
          <span className="ls-label">Total Leads</span>
        </div>
        {Object.entries(statusCounts).map(([status, count]) => {
          const cfg = STATUS_CONFIG[status] || {};
          return (
            <div key={status} className="lead-summary-card" style={{ borderColor: cfg.color + '33' }}>
              <span className="ls-value" style={{ color: cfg.color }}>{count}</span>
        ...
        <div className="source-funnel">
          {sourceCounts.map(([source, count]) => (
            <div key={source} className="funnel-row">
              <span className="funnel-source">
                <span className="source-dot" style={{ background: SOURCE_COLORS[source] || 'var(--text-muted)' }} />
                {source}
              </span>
              <div className="funnel-bar-wrap">
                <div
                  ...
                  style={{
                    ...
                    background: SOURCE_COLORS[source] || 'var(--text-muted)',
                  }}
                />
              </div>
      ...
      <Card title="All Leads" subtitle={`${filtered.length} of ${leads.length} shown`}>
        <div className="lead-filters">
          ...
          {(filterSource !== 'All' || filterStatus !== 'All' || filterService !== 'All' || search) && (
            <button className="btn btn-ghost lead-clear" onClick={() => { setFilterSource('All'); setFilterStatus('All'); setFilterService('All'); setSearch(''); }}>
              Clear
            </button>
          )}
        </div>
        ...
        {filtered.length === 0 ? (
          <p className="no-results">No leads match the current filters.</p>
        ) : (
          <table className="data-table">
            <thead>
              <tr>
              ...
              {filtered.map(lead => {
                ...
                return (
                  <tr key={lead.id}>
                    ...
                    <td>
                      <span className="source-pill" style={{ color: SOURCE_COLORS[lead.source] || 'var(--text-secondary)', background: (SOURCE_COLORS[lead.source] || 'var(--text-secondary)') + '18' }}>
                        <span className="source-dot-sm" style={{ background: SOURCE_COLORS[lead.source] || 'var(--text-muted)' }} />
                        {lead.source}
                      </span>
                    </td>
    ...
    <div className="analytics">
      ...
      <div className="analytics-tabs">
        {TABS.map(t => (
          <button
            key={t.id}
            className={`analytics-tab ${activeTab === t.id ? 'analytics-tab--active' : ''}`}
